...
ChannelList *chan;
{
    ...
    if (server_list[from_server].SZUnban < 2)
        server_list[from_server].SZUnban = 3;
    else server_list[from_server].SZUnban += 2;
...
send_begin:
        ...
        if (p) {
            if (p == completing) *dir = '/';
                ...
                strmcpy(filepath, dir, sizeof(filepath));
            }
        ...
        if (!(*dir)) {
            ...
            if (p) {
                ...
                strmcpy(filepath, p, sizeof(filepath));
                new_free(&p);
            }
            else {
        ...
        while (j < n) {
            if (!strncmp(dir_list[j]->d_name, file, strlen(file))) {
                ...
                strmcat(filepath, "/", sizeof(filepath));
