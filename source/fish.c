...
void KeyExchange(char *command, char *args, char *subargs)
{
    char *target;
...
/* Retrieve their key */
void FishAddRemoteKey(char *nick, char *notice)
{
...
int FishDecrypt(char *dest, char *src, char *key, int bufsize)
{
    int len;
...
/* type = 0 ... private msg
          1 ... public msg */
int FishEncrypt(char *dest, char *src, char *key, int bufsize, int type)
{
    int len = strlen(src);
    int i = 0, j, padlen;
    int dopad = 1;
